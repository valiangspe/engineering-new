...
app.MapPost("/parse-pdf", async (HttpContext context, AppDbContext dbContext) =>
{
    // Read JSON body
    var requestBody = await JsonSerializer.DeserializeAsync<Base64PdfRequest>(context.Request.Body);

    if (requestBody?.File == null)
    {
        return Results.BadRequest("Invalid or missing Base64 file.");
    }

    try
    {
        // Decode Base64 string
        string base64Data = requestBody.File; // Remove the "base64_" prefix
        ...
        string savedFilePath = Path.Combine(uploadsDir, filename);
        await File.WriteAllBytesAsync(savedFilePath, pdfBytes);

        var sldDocument = new SLDDocument
        {
            Filename = filename,
            Items = new List<SLDDocumentItem>()
        };

        // Save SLDDocument to generate ID
        dbContext.SLDDocuments.Add(sldDocument);
        await dbContext.SaveChangesAsync();

        // Count pages using ImageMagick
        int pageCount = BitmapClassProcessor.GetPdfPageCount(savedFilePath);

        int pageIndex = 0;
        for (int i = 0; i < pageCount; i++)
        {
            string imagePath = Path.Combine(uploadsDir, $"{sldDocument.Id}_{pageIndex}.png");

            // Render PDF page to image using ImageMagick
            BitmapClassProcessor.RenderPdfPageToBitmap(savedFilePath, i, imagePath);

            // Create and save SLDDocumentItem
            var documentItem = new SLDDocumentItem
            {
                SLDDocumentId = sldDocument.Id
            };
            sldDocument.Items.Add(documentItem);

            dbContext.SLDDocumentItems.Add(documentItem);
            pageIndex++;
        }

        await dbContext.SaveChangesAsync();
        return Results.Ok(sldDocument);
    }
    catch (Exception ex)
    {
...
namespace SupportReportAPI.Models
{
    ...
    public class SLDDocument : BaseModel
    {
        [Key]
        public int? Id { get; set; }
        public string? Filename { get; set; }
        public List<SLDDocumentItem>? Items { get; set; }

    }
    public class SLDDocumentItem : BaseModel
    {
        [Key]
        public int? Id { get; set; }
        public SLDDocument SLDDocument { get; set; }
        public int? SLDDocumentId { get; set; }
    }
    ...
    public class AppDbContext : DbContext
    {
        ...
        public DbSet<SLDDocument> SLDDocuments { get; set; }
        public DbSet<SLDDocumentItem> SLDDocumentItems { get; set; }
...
public class BitmapClassProcessor
{
    public static void RenderPdfPageToBitmap(string pdfPath, int pageIndex, string outputImagePath)
    {
        string imagemagickExecutable = "convert"; // Ensure ImageMagick 'convert' is installed and in PATH
        string arguments = $"{pdfPath}[{pageIndex}] -quality 100 -density 300 -depth 8 {outputImagePath}";

        Console.WriteLine($"{imagemagickExecutable} {arguments}");
    ...
    public static int GetPdfPageCount(string pdfPath)
    {
        ...
        string arguments = $"{pdfPath} dump_data";

        var process = new System.Diagnostics.Process
        {
            StartInfo = new System.Diagnostics.ProcessStartInfo
            {
                FileName = pdftkExecutable,
                Arguments = arguments,
                RedirectStandardOutput = true,
                RedirectStandardError = true,
                UseShellExecute = false,
        ...
        {
            string errorOutput = process.StandardError.ReadToEnd();
            throw new Exception($"pdftk failed to count pages: {errorOutput}");
        }

        // Parse page count from the output
        var lines = output.Split('\n');
        var pageCountLine = lines.FirstOrDefault(line => line.StartsWith("NumberOfPages"));
        if (pageCountLine != null && int.TryParse(pageCountLine.Split(':')[1].Trim(), out int pageCount))
        {
            return pageCount;
        }

        throw new Exception("Failed to parse page count from pdftk output.");
    }
}



// Helper class to parse JSON request
