...
app.MapPost("/panelprocess", async (PanelProcess panelProcess, AppDbContext db) =>
{
    ...
    {
        Console.WriteLine("Error creating PanelProcess: " + ex.Message);
        return Results.Problem("An error occurred while creating the PanelProcess.");
    }
});
...
app.MapGet("/panelprocess", async (AppDbContext db) =>
{
    try
    {
        var panelProcesses = await db.PanelProcesses.ToListAsync();
        return Results.Ok(panelProcesses);
    }
    catch (Exception ex)
    {
        Console.WriteLine("Error fetching PanelProcesses: " + ex.Message);
        return Results.Problem("An error occurred while fetching the PanelProcesses.");
    }
});
...
app.MapGet("/panelprocess/{id}", async (AppDbContext db, int id) =>
{
    ...
    {
        Console.WriteLine("Error fetching PanelProcess by ID: " + ex.Message);
        return Results.Problem("An error occurred while fetching the PanelProcess.");
    }
});
...
app.MapPut("/panelprocess/{id}", async (int id, PanelProcess panelProcess, AppDbContext db) =>
{
    ...
    {
        Console.WriteLine("Error updating PanelProcess: " + ex.Message);
        return Results.Problem("An error occurred while updating the PanelProcess.");
    }
});
...
app.MapDelete("/panelprocess/{id}", async (AppDbContext db, int id) =>
{
    ...
    {
        Console.WriteLine("Error deleting PanelProcess: " + ex.Message);
        return Results.Problem("An error occurred while deleting the PanelProcess.");
    }
});
...
app.MapPut("/api/notifications/update-task-status", async (int taskId, string role, AppDbContext db) =>
{
    // Cari notifikasi terkait taskId
    var notification = await db.Set<Notification>().FirstOrDefaultAsync(n => n.TaskId == taskId && n.Role == role);
    ...
    if (nextRole != null)
    {
        ...
        {
            Title = "Task Update",
            Message = $"Task ID {taskId} siap untuk dilakukan done oleh {nextRole.ToUpper()}.",
            TaskId = taskId,
            Role = nextRole,
    ...
    {
        // Tandai notifikasi sebagai completed jika semua role selesai
...
{
    var notification = await db.Set<Notification>().FindAsync(id);
    if (notification == null) return Results.NotFound();
...
public class Notification : BaseModel
{
    [Key]
    public int Id { get; set; } // Primary key
    public string Title { get; set; } = string.Empty; // Judul notifikasi
    public string Message { get; set; } = string.Empty; // Pesan notifikasi
    public int TaskId { get; set; } // ID task terkait
    public string Role { get; set; } = string.Empty; // Role penerima (PIC, SPV, Manager)
    public bool IsRead { get; set; } = false; // Status apakah sudah dibaca
    public string Status { get; set; } = "Not Completed"; // Status notifikasi: OnGoing atau Completed
    public DateTime CreatedAt { get; set; } // Waktu notifikasi dibuat
}
