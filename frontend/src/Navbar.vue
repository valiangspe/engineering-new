<template>
  ...
  >
    <v-list density="compact" nav>
      ...
      ></v-list-item>
      <v-list-item
        href="/#/SupportEngineeringDocument"
        prepend-icon="mdi-file-cog-outline"
        title="Support Engineering Doc"
      ></v-list-item>
      <v-list-item
        href="/#/ecn"
      ...
      >
        <template v-slot:append>
          <v-icon>{{
            isReportExpanded ? "mdi-chevron-up" : "mdi-chevron-down"
          }}</v-icon>
        </template>
      </v-list-item>
  ...
  if (userId) {
    try {
      const roleResp = await axios.get(
        `${import.meta.env.VITE_APP_BASE_URL}/api/userroles/byUserId/${userId}`
      );
